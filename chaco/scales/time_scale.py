...
def tfrac(t, **time_unit):
    ...
    if len(time_unit) > 1:
        raise ValueError(
            "tfrac() only takes one keyword argument, got %d" % len(time_units)
        )
...
HMSScales = (
    [TimeScale(microseconds=1), TimeScale(milliseconds=1)]
...
MDYScales = [
    TimeScale(day_of_month=list(range(1, 31, 3))),
