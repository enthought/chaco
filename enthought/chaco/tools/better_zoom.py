...
class BetterZoom(BaseTool, ToolHistoryMixin):
    ...
    def zoom_out(self):
        if self.axis != 'value':
            self._zoom_out_mapper(self.component.index_mapper)
        if self.axis != 'index':
            self._zoom_out_mapper(self.component.value_mapper)
            
        self.component.request_redraw()    
    
    def _zoom_in_mapper(self, mapper):
        high = mapper.range.high
        low = mapper.range.low
        range = high-low
        
        new_range = range/self.zoom_in_increment
        mapper.range.high -= new_range/2
        mapper.range.low += new_range/2

    def _zoom_out_mapper(self, mapper):
        high = mapper.range.high
        low = mapper.range.low
        range = high-low
        
        mapper.range.high += range/2
        mapper.range.low -= range/2
    ...
    def normal_key_pressed(self, event):
        ...
        if self.zoom_in_key.match(event):
            ...
            self._zoom_in_mapper(self._get_x_mapper())
            event.handled = True
        elif self.zoom_out_x_key.match(event):
            self._zoom_out_mapper(self._get_x_mapper())
            event.handled = True
        elif self.zoom_in_y_key.match(event):
            self._zoom_in_mapper(self._get_y_mapper())
            event.handled = True
        elif self.zoom_out_y_key.match(event):
            self._zoom_out_mapper(self._get_y_mapper())
            event.handled = True
