...
class BarPlot(AbstractPlotRenderer):
    ...
    # Type of width used for bars -- if "data" is selected, then the width is
    # in the units along the x-dimension of the data space.  If "screen" is
    # selected, the bar_width uses a fixed width of pixels.
    bar_width_type = Enum("data", "screen")
    
    # Width of the bars, in data or screen space
    bar_width = Float(10)
    
    # Switch to round or not on rectangle dimensions--not strictly an
    #  'antialias' switch, but has the same effect through exact pixel drawing
    antialias = Bool(True)
