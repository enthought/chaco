...
class LineInspector(BaseTool):
    """
    Simple tool to draw a line parallel to the index or the value axis of
    an X-Y plot.  Only support plots with 1-D index.
    """
    ...
    # Should the tool respond to user mouse events?
    is_interactive = true
    
    # Should the tool respond to updates in the metadata on the datasource
    # and update its own position?
    is_listener = false
    
    # If interactive, should the line inspector write the current dataspace point
    # to the appropriate datasource's "selections" metadata?
    write_metadata = false
    
    # The name of the metadata field to listen/write to 
    ...
    # Private trait recording the position where we last saw the mouse
    ...
    def _get_screen_pts(self):
        ...
        our plot component as a tuple (x, y).  A dimension that doesn't
        have a selected point will have the value None at its index
        in the tuple, or won't have the key.
        """
        plot = self.component
        if plot is None:
            return
    ...
    def _map_to_data(self, x, y):
        """
        Returns the dataspace coordinates of the given x and y.  Takes into
        account orientation of the plot and our axis setting.
        """
    ...
    def _draw_horizontal_line(self, gc, sy):
        """
        Draws a horizontal line through screen point (sx,sy) having the width
        of our component
        """
